...
const FilterTree = ({ slug, show, setShow, setCategoryIds }) => {
    ...
    const handleAllCategoryExpansion = (expandedKeysArg, { expanded, node }) => {
        const parentKey = findParentKey(treeData, node.key);

        if (!expanded) {
            // Node was collapsed, so just remove it from expandedKeys
            setExpandedKeys(prev => prev.filter(k => k !== node.key));
        } else {
            // Node was expanded — collapse siblings
            const siblingKeys = getSiblingKeys(treeData, parentKey).filter(k => k !== node.key);
            setExpandedKeys(prev =>
                [...prev.filter(k => !siblingKeys.includes(k)), node.key]
            );
        }
    };

    const findParentKey = (nodes, childKey, parent = null) => {
        for (let node of nodes) {
            if (node.key === childKey) return parent?.key || null;
            if (node.children?.length) {
                const found = findParentKey(node.children, childKey, node);
    ...
    const getSiblingKeys = (nodes, parentKey) => {
        if (!parentKey) return []; // top level
        const parentNode = findNodeById(nodes, parentKey);
        return parentNode?.children?.map(child => child.key) || [];
    };
