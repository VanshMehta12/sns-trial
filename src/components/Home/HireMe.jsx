...
import HireMeItems from "./HireMeItems";
...
const HireMe = () => {
    ...
    const getCategoriesData = async (page) => {
        const extractCategoriesAndSubcategories = (categories) => {
            const result = [];
            categories.forEach(cat => {
                result.push(cat); // Add the main category
                if (Array.isArray(cat.subcategories)) {
                    result.push(...cat.subcategories); // Add its subcategories
                }
            });
            return result;
        };
        ...
        if (prevLang?.id !== CurrentLanguage?.id) {
            ...
            try {
                ...
                if (data && Array.isArray(data.data)) {
                    const flattened = extractCategoriesAndSubcategories(data.data);

                    setCachedData(prev => ({
                        ...prev,
                        [page]: flattened
                    }));
                    setCatData(flattened);

                    if (page > catCurrentPage) {
                        dispatch(setCateData(flattened));
            ...
            try {
                ...
                if (data && Array.isArray(data.data)) {
                    ...
                    setCatData(Object.values(newCached).flat());

                    if (page > catCurrentPage) {
                        dispatch(setCateData([...cateData, ...flattened]));
                        dispatch(setCatCurrentPage(data?.current_page));
                        dispatch(setCatLastPage(data?.last_page));
                    }
                    setLastPage(data.last_page);
                    ...
                    <>
                        <div className="row mrg_btm">
                            <div className="col-12">
                                ...
                                {cateData.filter((ele) => ele.hire_me === "1").length > 0 && (
                                    <>
                                        <div className="pop_cat_header">
                                            <h4 className="pop_cat_head">{t('hireMe')}</h4>
                        ...
                        <div className="row">
                            <div className="col-12">
                                ...
                                >
                                    {cateData
                                        .filter((ele) => ele.hire_me === "1")
                                        .map((ele, index) => (
                                            <SwiperSlide key={index} className="popular-category-slide">
                                                <HireMeItems data={ele} />
                                            </SwiperSlide>
                                        ))}
